...
export default function ProductProfile() {
  ...
  const handleAddToCart = (product) => {
    ...
    if (!isExist) {
      ...
      // 🔔 Event fire karo taaki Header turant update ho
    ...
    <div className="container py-5">
      <div className="row g-5">
            ...
            {[...Array(5)].map((_, i) => (
              <img
                ...
                style={{
                  ...
                  height: "70px",
                  objectFit: "cover",
                  border: previewImage.includes(product.image) ? "2px solid #28a745" : "1px solid #ddd",
                  cursor: "pointer",
        ...
        <div className="col-md-6">
          <h2 className="fw-bold text-capitalize mb-2">{product.prname}</h2>
          <p className="text-muted fs-5 mb-3">{product.prtitle}</p>

          <p className="mb-1">
            <strong>Brand:</strong> {product.prbrand}
          </p>

          <p className="text-success fs-4 fw-bold mt-3">
            ₹{product.prprice}
            <small className="text-muted text-decoration-line-through ms-2">₹{parseInt(product.prprice) + 500}</small>{" "}
            <span className="text-danger">{Math.round((500 / (parseInt(product.prprice) + 500)) * 100)}% off</span>
          </p>
          ...
          <div className="d-flex gap-3 mt-4">
            <button onClick={() => handleAddToCart(product)} className="btn btn-success fw-bold px-4 py-2">
...
//           <p className="mb-1">
//             <strong>Title:</strong> {product.prbrand}
//           </p>
